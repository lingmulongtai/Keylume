...
| チェック                | 結果              | 対象                                                                                                      |
| ----------------------- | ----------------- | --------------------------------------------------------------------------------------------------------- |
| Rust unit tests         | 25 passed         | プロトコル、量子化、ブレンド、ゾーン、全エフェクト、保存復旧、入力転送、プロファイル、出力キュー、解放    |
...
- 公開前レビューに対応し、MIDI ドライバー停止時のキュー投入・終了待ちを制限し、低サンプルレートで FFT の負周波数側を参照しないよう修正しました。
